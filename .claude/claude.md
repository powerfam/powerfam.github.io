...
title: "제목"
date: 2025-MM-DD
description: "한 줄 설명 (페이지 메타데이터용)"
summary: "40자 이하 요약 (카드뉴스 표시용)"
tags:
  - 편지
  - 추가태그
section: section1  # 또는 section3
---
```

### Summary 작성 규칙
- **40자 이하** 필수 (카드뉴스 UI 제약)
- 핵심 메시지를 한 문장으로 압축
- 예시: "목표가 명확하면 고통도 의미가 생긴다"
...
1. **소제목**: `##` 마크다운 헤더 사용
2. **강조**: `<b>텍스트</b>` HTML 태그 사용 (마크다운 `**` 대신)
3. **인용문**: 글 끝에 `>` blockquote로 철학자/저자 명언 추가
4. **Footer**: section1만 하단에 소개문 추가 (section3은 제외)

### 문단 스타일

- 편지체 특성상 짧은 호흡의 문장 허용
- 의미 단위로 빈 줄 구분
- 과도한 엔터 지양 (2-3문장은 한 문단으로 결합 가능)
- Non-fiction 에세이 스타일 유지

### Section 구분
- **section1**: 삶, 직장생활, 인간관계, 성장, 마인드셋
- **section3**: 투자, 금융, 돈, 경제

### 이미지 삽입
```markdown
![이미지 설명](/images/파일명.확장자)
```
